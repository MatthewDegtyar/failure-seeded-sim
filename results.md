# results.md — Failure-Seeded Simulation: Validation Experiment

Date: 2026-05-22
Engine: MuJoCo 3.8.1 (CPU). Optimizer: CMA-ES (`cma` 4.4.4).
All experiments ran locally on a laptop CPU. **AWS spend used: $0.00.**
...
All random seeds are explicit in each script. All artifacts land in
`data/` (`.npz` + `.json`) and `analysis/plots/`.

## Spend

| line item | cost |
|---|---|
| AWS spot instances | $0.00 (not provisioned) |
| Local CPU time | ~10 min wall |
| **Total cloud spend** | **$0.00 / $200 budget** |
